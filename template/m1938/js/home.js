...
var MAC={
	...
	'Cookie': {
		...
		'Delone': function(name){
			var alist = $('#history_box').find('a');
			for(i=0;i<alist.length;i++){
				if($('#history_box').find('a').eq(i).html()==name){
					$('#history_box').find('a').eq(i).parent().remove();
					break;
				}
			}
			var str = MAC.Cookie.Get('mac_history');
			//alert(str);
			var obj = eval(str);
			...
			for($j=0; $j<=10; $j++){
				if(obj[$j]){
					jsonstr += '{"name":"'+obj[$j].name+'","link":"'+obj[$j].link+'","typename":"'+obj[$j].typename+'","typelink":"'+obj[$j].typelink+'","pic":"'+obj[$j].pic+'"},';
				}
			}
			jsonstr = jsonstr.substring(0,jsonstr.lastIndexOf(','));
			jsonstr += "]}";
			MAC.Cookie.Set('mac_history',jsonstr,7);
		}
	},
	'Login':{
		'Display': true,
	/*	'Init':function($id){
		...
		'Create':function($id){
			...
			if(MAC.Cookie.Get('userid')!=undefined && MAC.Cookie.Get('userid')!=''){ 
				username = MAC.Cookie.Get('username');
				//alert(username);
				$('#login').html(username);
		...
		'Init':function($id){
			if(MAC.Cookie.Get('userid')){
				$('#login').html('<span><a target="_blank" href="'+SitePath+'index.php?m=user-index">'+decodeURI(MAC.Cookie.Get('usernickname'))+'</a></span><a href="javascript:;" onclick="MAC.Login.Out();">[退出]</a>');
				$('#loginindex').html('<div class="hd_fBtn hd_fBtn5"><span><a target="_blank" href="'+SitePath+'index.php?m=user-index">'+decodeURI(MAC.Cookie.Get('usernickname'))+'</a></span><a href="javascript:;" onclick="MAC.Login.Out();">[退出]</a></div>');
			}
		...
		'Out':function($id){
			$.ajax({type:'get',url: SitePath + 'index.php?m=user-logout',timeout: 5000,
				success: function($r){
					MAC.Cookie.Set('userid','',0);
					MAC.Cookie.Set('username','',0);
					MAC.Cookie.Set('usernickname','',0);
					$('#login').html('<a href="javascript:;">登录</a>');
					$('#loginindex').html('<div class="hd_fBtn hd_fBtn1"><a href="#" class="hd_xian">登录</a> | <a target="_blank" href="{maccms:path}index.php?m=user-reg.html">注册</a></div>');
				}
			});
		},
		'In':function(){
			...
			$.ajax({type: 'post',url: SitePath + 'index.php?m=user-check', data:'u_name='+$("#username").val()+'&u_password='+$("#userpass").val(),timeout: 5000,
				...
				success: function($r){
					if($r.indexOf('您输入')>-1){
						...
						MAC.Cookie.Set('username',$("#username").val(),1); 
						$('#login').html('<span><a target="_blank" href="'+SitePath+'index.php?m=user-index">'+MAC.Cookie.Get('usernickname')+'</a></span><a href="javascript:;" onclick="MAC.Login.Out();">[退出]</a>');
						$('#loginindex').html('<div class="hd_fBtn hd_fBtn5"><span><a target="_blank" href="'+SitePath+'index.php?m=user-index">'+MAC.Cookie.Get('usernickname')+'</a></span><a href="javascript:;" onclick="MAC.Login.Out();">[退出]</a></div>');
	...
	'History': {
		...
		'Clear': function(){
			...
			$('.hd_list ul').html('<li class="hx_clear">已清空观看记录。</li>');
		},	
		'Show': function(){
			$('#history_box').show();
		},
		...
		'Create': function($id){
			...
			html = '<div class="fixdh_fbtn"></div><div class="hd_hist" id="history_box"><span class="hd_fArr"></span><div class="hd_tab"><div class="hd_on l"><a href="#">全部</a></div><span class="r c-grey his-btn" onclick="MAC.History.Clear();">清空记录</span></div><div class="hd_hist_tab"><div class="hd_list"><ul>';
			if(jsondata.length > 0){
				for($i=0; $i<jsondata.length; $i++){
					html +='<li class="his_item"><a href="'+jsondata[$i].link+'" target="_blank">'+jsondata[$i].name+'</a><p class="action"><a title="删除" href="javascript:;"  onclick="MAC.Cookie.Delone(\''+jsondata[$i].name+'\');" class="h-btn r c-red">删除</a><a href="'+jsondata[$i].link+'" target="_blank" class="c-red">继续播放</a></p></li>';
					//html +='<a href="javascript:;" onclick="MAC.Cookie.Delone(\''+jsondata[$i].name+'\');" class="hx_title">'+jsondata[$i].name+'</a></dd>';
				}
			}else{
				html +='<dd class="hide">暂无观看记录</dd>';
			}
			html += '</ul></div></div><p class="hd_mb">登录可永久保存观看记录,并同步到各终端</p><div class="hd_cf"><a href="'+SitePath+'index.php?m=user-login.html" class="hd_btn l">点击登录</a><a href="'+SitePath+'index.php?m=user-reg.html" class="hd_btngrey r">免费注册</a></div></div>';
			$('#'+$id).after(html);
	...
	'Dialogue':{
		...
		'Post':function(){
			if($("#c_dialogue").val() == '欢迎分享该剧的经典对白，最多200个字。'){
				alert('请发表您的评论观点！');
				return false;
			}
			$.ajax({
				...
				data: "userid="+$("#userid").val()+"&vid="+SiteId+"&username="+$("#c_name").val()+"&dialogue="+$("#c_content").val()+"&c_code="+$("#c_code").val(),
				success:function($r){
					if($r == 'ok'){
						MAC.Dialogue.Show(SitePath+'index.php?m=dialogue-show-vid-'+SiteId);
					}
					else{
						alert($r);
					}
		...
		'Reply':function($id){
			$("#c_rid").val($id);
			window.scrollTo(0, $("#c_dialogue").offset().top-30);
		}
	},
	'Evaluation':{
		'Show':function($ajaxurl){
		...
		'Post':function(){
			if($("#c_evaluation").val() == '欢迎分享该剧的精彩影评，最多200个字。'){
				alert('请发表您的评论观点！');
				return false;
			}
			$.ajax({
				...
				data: "userid="+$("#userid").val()+"&vid="+SiteId+"&username="+$("#c_name").val()+"&evaluation="+$("#c_content").val()+"&c_code="+$("#c_code").val(),
				success:function($r){
					if($r == 'ok'){
						MAC.Evaluation.Show(SitePath+'index.php?m=evaluation-show-vid-'+SiteId);
					}
					else{
						alert($r);
					}
		...
		'Reply':function($id){
			$("#c_rid").val($id);
			window.scrollTo(0, $("#c_evaluation").offset().top-30);
		}
	},
	'Lazyload':{
		'Show': function(){
